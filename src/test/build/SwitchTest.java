...
class SwitchTest {
    ...
    void setUp() {
        ...
        testDevice1 = new Device("DEV001", "192.168.1.2", "NYC");
        testDevice2 = new Device("DEV002", "192.168.1.3", "NYC");
    }
    ...
    void shouldManageValidConnections() {
        assertTrue(networkSwitch.addDevice(1, testDevice1), "Should connect to valid port");
        ...
        assertTrue(networkSwitch.addDevice(2, testDevice2), "Should connect second device");
    ...
    void shouldRejectInvalidPorts(int invalidPort) {
        assertFalse(networkSwitch.addDevice(invalidPort, testDevice1), "Should reject invalid port");
    ...
    void shouldHandlePortReuse() {
        assertTrue(networkSwitch.addDevice(1, testDevice1), "Should connect first device");
        assertFalse(networkSwitch.addDevice(1, testDevice2), "Should reject duplicate port use");
        assertEquals(testDevice1, networkSwitch.getConnectedDevice(1), "Should maintain original connection");

        assertTrue(networkSwitch.removeDevice(1), "Should disconnect device");
        assertTrue(networkSwitch.addDevice(1, testDevice2), "Should allow reconnection to freed port");
    ...
    void shouldProvideImmutableCollections() {
        networkSwitch.addDevice(1, testDevice1);
        networkSwitch.addDevice(2, testDevice2);

        Map<Integer, Device> portMap = networkSwitch.getPortMap();
    ...
    void shouldMaintainPortOrder() {
        networkSwitch.addDevice(5, testDevice1);
        networkSwitch.addDevice(2, testDevice2);
        ...
        Device testDevice3 = new Device("DEV003", "192.168.1.4", "NYC");
        networkSwitch.addDevice(3, testDevice3);
    ...
    void shouldHandleFullCapacity() {
        for (int i = 1; i <= PORT_COUNT; i++) {
            ...
            assertTrue(networkSwitch.addDevice(i, device),
        ...
        Device extraDevice = new Device("DEV999", "192.168.1.100", "NYC");
        assertFalse(networkSwitch.addDevice(1, extraDevice),
