...
class SwitchValidationTest {
    ...
    void setUp() {
        ...
        device1 = new Device("HOST001", "10.0.1.10", "LAX");
        device2 = new Device("HOST002", "10.0.1.11", "LAX");
        device3 = new Device("HOST003", "10.0.1.12", "LAX");
    }
    ...
    void shouldRejectInvalidPorts(int invalidPort) {
        assertFalse(networkSwitch.addDevice(invalidPort, device1), "Should reject connection to invalid port: " + invalidPort);
        assertNull(networkSwitch.getConnectedDevice(invalidPort), "Should not have device on invalid port: " + invalidPort);

        assertFalse(networkSwitch.removeDevice(invalidPort), "Should reject disconnection from invalid port: " + invalidPort);
    ...
    void shouldHandlePortConnections() {
        assertTrue(networkSwitch.addDevice(1, device1), "Should connect to first port");
        assertEquals(device1, networkSwitch.getConnectedDevice(1), "Should get device from first port");

        assertTrue(networkSwitch.addDevice(PORT_COUNT, device2), "Should connect to last port");
        assertEquals(device2, networkSwitch.getConnectedDevice(PORT_COUNT), "Should get device from last port");

        assertTrue(networkSwitch.addDevice(PORT_COUNT / 2, device3), "Should connect to middle port");
    ...
    void shouldHandlePortDisconnections() {
        ...
        networkSwitch.addDevice(PORT_COUNT, device2);
        networkSwitch.addDevice(PORT_COUNT / 2, device3);
        assertEquals(3, networkSwitch.getOccupiedPorts().size(), "Should start with three connections");

        assertTrue(networkSwitch.removeDevice(1), "Should disconnect from first port");
        assertNull(networkSwitch.getConnectedDevice(1), "First port should be empty");

        assertTrue(networkSwitch.removeDevice(PORT_COUNT), "Should disconnect from last port");
        assertNull(networkSwitch.getConnectedDevice(PORT_COUNT), "Last port should be empty");

        assertTrue(networkSwitch.removeDevice(PORT_COUNT / 2), "Should disconnect from middle port");
    ...
    void shouldHandlePortReuse() {
        assertTrue(networkSwitch.addDevice(1, device1), "Should connect first device");
        assertTrue(networkSwitch.removeDevice(1), "Should disconnect first device");
        assertTrue(networkSwitch.addDevice(1, device2), "Should connect second device to same port");
        assertEquals(device2, networkSwitch.getConnectedDevice(1), "Should get new device from reused port");

        for (int i = 0; i < 5; i++) {
            assertTrue(networkSwitch.removeDevice(1), "Should disconnect in cycle " + i);
            assertTrue(networkSwitch.addDevice(1, device1), "Should connect device1 in cycle " + i);
            assertTrue(networkSwitch.removeDevice(1), "Should disconnect in cycle " + i);
            assertTrue(networkSwitch.addDevice(1, device2), "Should connect device2 in cycle " + i);
        }
    }
    ...
    void shouldHandleMultipleOperations() {
        for (int i = 2; i <= PORT_COUNT; i += 2) {
            assertTrue(networkSwitch.addDevice(i, new Device("HOST" + i, "10.0.1." + i, "LAX")), "Should connect to port " + i);
        }
        ...
        for (int i = 1; i <= PORT_COUNT; i++) {
            assertTrue(networkSwitch.removeDevice(i), "Should disconnect port " + i);
        }
        assertEquals(PORT_COUNT, networkSwitch.getAvailablePorts(), "All ports should be available");
        assertTrue(networkSwitch.getOccupiedPorts().isEmpty(), "No ports should be occupied");
    }
    ...
    void shouldMaintainPortOrder() {
        networkSwitch.addDevice(5, device1);
        networkSwitch.addDevice(2, device2);
        networkSwitch.addDevice(8, device3);
        ...
        networkSwitch.removeDevice(5);
        networkSwitch.addDevice(3, device1);
    ...
    void shouldHandleNullDevice() {
        assertFalse(networkSwitch.addDevice(1, null), "Should reject null device");
        ...
        networkSwitch.addDevice(1, device1);
        assertFalse(networkSwitch.addDevice(2, null), "Should reject null device after valid connection");
